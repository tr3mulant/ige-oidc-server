...
describe('the root', function () {
    test('sends a guest to the login page', function () {
        $this->get('/')->assertRedirect(route('login'));
    });

    test('sends a signed-in person to their account security screen', function () {
        $this->actingAs(User::factory()->twoFactorEnabled()->create())
            ->get('/')
            ->assertRedirect(route('account.security'));
    });

    test('serves no welcome page to anyone', function () {
     ...
     * by nothing else.
     */
    test('ignores a redirect parameter rather than honouring it', function (string $query) {
     ...
     */
    test('sends an un-enrolled person to enrollment instead', function () {
...
describe('the account security screen', function () {
    test('sends a guest to login', function () {
     ...
     */
    test('opens without a further password prompt', function () {
        $this->actingAs(User::factory()->twoFactorEnabled()->create())
            ->get(route('account.security'))
            ->assertOk();
    });

    test('shows the recovery codes to nobody, only a link to them', function () {
    ...
    test('names the person and the username issued as their claim', function () {
        $user = User::factory()->twoFactorEnabled()->create([
            'name' => 'Robin Vance',
            'username' => 'robinvance',
        ]);
...
describe('recovery codes', function () {
    ...
    test('shows every code once the password is confirmed', function () {
        $user = User::factory()->twoFactorEnabled()->create();
    ...
    test('regenerating replaces every code', function () {
        $user = User::factory()->twoFactorEnabled()->create();
...
describe('moving to a new device', function () {
    test('requires a confirmed password to even see the confirmation step', function () {
        $this->actingAs(User::factory()->twoFactorEnabled()->create())
     ...
     */
    test('is where the password confirmation returns you, ready to act', function () {
        $user = User::factory()->twoFactorEnabled()->create();
     ...
     */
    test('clears the enrollment and forces a fresh one', function () {
        $user = User::factory()->twoFactorEnabled()->create();
...
describe('changing a password', function () {
    test('the form is on the account security screen', function () {
        $this->actingAs(User::factory()->twoFactorEnabled()->create())
            ->get(route('account.security'))
            ->assertSee(route('user-password.update'));
    });

    test('it changes the password when the current one is given', function () {
        $user = User::factory()->twoFactorEnabled()->create();
     ...
     */
    test('it refuses without the current password, into the bag the form reads', function () {
        $user = User::factory()->twoFactorEnabled()->create();
